...
library = [
  ...
  (InfixL, show OpSwizzle, "'a 'n -> Real 'm -> 'a 'm", "swizzle, return vector of all subscripts supplied (all ns must be statically determinable; bounds checking occurs)", False, ["as", "ns"]),
  ...
  (Prefix, "ceiling", "Real -> Real", "round to positive infinity", False, ["x"]),
