...
library = [
  (Prefix, "negate", sig "Real -> Real", "scalar negate (as desugared from \"-\")", False, ["x"]),
  (Prefix, "negates", sig "Real n -> Real n", "vector negate (component-wise) (as desugared from \"--\")", False, ["x"]),
  (Prefix, "~", sig "Bool -> Bool", "logical not", False, ["x"]),
  (InfixL, "!", sig "a n -> Real -> 'a", "subscript (if n is statically determinable, bounds checking occurs; if n is not statically determinable, there is no bounds check, and the left argument must be a uniform)", False, ["as", "n"]),
  (InfixL, "!!", sig "a n -> Real m -> 'a m", "swizzle, return vector of all subscripts supplied (if ns are all statically determinable, bounds checking occurs; if any n is not statically determinable, there is no bounds check, and the left argument must be a uniform) ", False, ["as", "ns"]),
  (InfixL, "+", sig "Real -> Real -> Real", "scalar add", False, ["x", "y"]),
  (InfixL, "-", sig "Real -> Real -> Real", "scalar sub", False, ["x", "y"]),
  (InfixL, "*", sig "Real -> Real -> Real", "scalar mul", False, ["x", "y"]),
  (InfixL, "/", sig "Real -> Real -> Real", "scalar div", False, ["x", "y"]),
  (InfixL, "++", sig "Real -> Real -> Real", "vector add (component-wise)", False, ["x", "y"]),
  (InfixL, "--", sig "Real -> Real -> Real", "vector sub (component-wise)", False, ["x", "y"]),
  (InfixL, "**", sig "Real -> Real -> Real", "vector mul (component-wise)", False, ["x", "y"]),
  (InfixL, "//", sig "Real -> Real -> Real", "vector div (component-wise)", False, ["x", "y"]),
  (InfixL, "**.", sig "Real p -> Real -> Real p", "vector-scalar mul", False, ["x", "y"]),
  (InfixL, "//.", sig "Real p -> Real -> Real p", "vector-scalar div", False, ["x", "y"]),
  (InfixL, "#", sig "Real q p -> Real r q -> Real r p", "matrix-matrix linear algebraic mul", False, ["x", "y"]),
  (InfixR, "#.", sig "Real q p -> Real q -> Real p", "matrix-vector linear algebraic mul", False, ["x", "y"]),
  (InfixL, ".#", sig "Real q -> Real r q -> Real r", "vector-matrix linear algebraic mul", False, ["x", "y"]),
  (InfixN, "<", sig "Real -> Real -> Bool", "less than", False, ["x", "y"]),
  (InfixN, "<=", sig "Real -> Real -> Bool", "less than or equal", False, ["x", "y"]),
  (InfixN, ">", sig "Real -> Real -> Bool", "greater than", False, ["x", "y"]),
  (InfixN, ">=", sig "Real -> Real -> Bool", "greater than or equal", False, ["x", "y"]),
  (InfixN, "==", sig "'a -> 'a -> Bool", "equality", False, ["x", "y"]),
  (InfixN, "/=", sig "'a -> 'a -> Bool", "inequality", False, ["x", "y"]),
  (InfixL, "&&", sig "Bool -> Bool -> Bool", "logical and", False, ["x", "y"]),
  (InfixL, "||", sig "Bool -> Bool -> Bool", "logical or", False, ["x", "y"]),
  (Postfix, "'", sig "'a q p -> 'a p q", "transpose", False, ["x"]),
